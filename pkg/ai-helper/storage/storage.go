...
func (r *MongoDBRepository) SaveMessage(ctx context.Context, msg aihelper.Message) (aihelper.Message, error) {
	// 检查消息的创建时间
	...
	if err != nil {
		return aihelper.Message{}, fmt.Errorf("%w: %v", ErrFailedToSaveMessage, err)
	}
...
func (r *MongoDBRepository) GetMessagesBySessionID(ctx context.Context, sessionID string) ([]aihelper.Message, error) {
	// 构建查询条件
	...
	// 遍历结果
	var messages []aihelper.Message
	for cursor.Next(ctx) {
		var msg aihelper.Message
